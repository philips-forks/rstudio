...
Error addUser(User* pUser)	
{
   std::string cmdUserAdd;
   std::stringstream sGrpId;
   sGrpId << pUser->groupId;
   std::stringstream sUsrId;
   sUsrId << pUser->userId;
   cmdUserAdd = "useradd " ;
   cmdUserAdd = cmdUserAdd + " -u " + sUsrId.str() +" -g " + sGrpId.str() + " -p " + pUser->username +" -d "+ pUser->homeDirectory +" -m -G dsp -s /bin/bash "+ pUser->username;
   int result = std::system(cmdUserAdd.c_str());
   ...
   Error error = systemError(result, ERROR_LOCATION);
   error.addProperty("user-name", safe_convert::numberToString(pUser->username));
   return error;
}
